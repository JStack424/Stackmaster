...
The **1.1.8 local test candidate** targets the continuous Stackmaster display workload visible while the hammer is equipped. Pinned Valheim IL shows that `Player.OnInventoryChanged` performs vanilla recipe/piece refresh and placement-ghost recreation, while Stackmaster's nearby-resource work comes from the surrounding ordinary hammer HUD `CanBuild` loop rather than directly from that inventory callback. Display snapshots now pre-index item/quality totals, reuse stable scope and chest data within a one-second display window, and refresh only the player contribution after a pickup/drop while preserving a separate five-second absolute chest age. Action-time build, craft, upgrade, and Quick Grab Materials checks remain fresh and exact. Opening an already-sorted inventory no longer sends a no-op change notification. The build menu also gains a keyboard/mouse-only **Quick Grab Materials** hint whose configured modifier text defaults to **Left Alt + Click**. This candidate still requires Joe's live frame-time comparison; 1.1.5 remains production.
...
The mesh resolver uses the canonical `piece_workbench` prefab resolved through `ZNetScene`, verifies each loaded station's ZDO prefab identity, and is covered by the exact runtime compatibility gate. Mesh discovery is complete rather than subject to the ordinary nearby-action responsiveness cutoff. Read-only resource display still claims nothing. After any asynchronous ownership yield and again before mutation, actions recompute a single immutable scope snapshot and validate selected chest membership together with identity, access, use state, revisions, quantities, and ownership. Fresh action captures resolve current location and connected-workbench topology before any mutation. Display-only topology changes that emit no inventory event can remain visible only for the rest of the current one-second cache window; no unloaded station or chest is represented in a newly resolved capture.

Current 1.1.8 automated verification covers 151 pure-domain tests, 297 independent compatibility/API/IL checks, and 73 static/repository safety checks. In addition to the prior policy matrix, the suite now covers indexed-vs-raw display parity, player-only display refresh, absolute chest-cache age, exact inventory/build call paths, and the keyboard-only Quick Grab Materials hint lifecycle. Joe live-tested and confirmed the 1.1.5 crafting debit fix; the new 1.1.8 performance and hint changes remain live-unverified. Broader multiplayer behavior, dedicated-server behavior, workbench extensions, manual-access preemption, and wider topology/load-unload edge cases remain independently unverified.
