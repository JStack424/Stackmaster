...
Current 0.4.0 automated verification passes with zero compiler warnings/errors, 78/78 pure-domain tests, and 51/51 static/repository safety checks. Core connected-workbench storage behavior has been live-tested in Valheim. Multiplayer behavior, dedicated-server behavior, workbench extensions, and broader topology/load-unload edge cases have not yet been independently verified, so this release does not claim proven multiplayer safety.
