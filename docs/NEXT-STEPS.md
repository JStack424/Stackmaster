...
- Local 0.2.0 Test Build 6 is the current unpublished candidate. It reads ownership-independent serialized chest snapshots for display/planning, closes Test Build 5's null live-inventory path, fails open around nearby-resource query/HUD patches, and requests only the minimum remotely owned chests required by an exact player-first action plan.
...
- Automated verification for Test Build 6 passes: 51/51 pure-domain tests and 43/43 static/repository checks, with zero compiler warnings/errors.
...
**Required 0.2.0 Test Build 6 live checks:**
...
**Next handoff:** Install the local 0.2.0 Test Build 6 ZIP and first confirm chest opening plus the build/crafting menus no longer throw, then begin checks 1–5 using two players so ownership can be observed directly. Do not change the live Thunderstore package or GitHub release until Joe explicitly approves a later publication step.
