...
The **1.1.9 local test candidate** preserves the completed 1.1.8 selective requirement-display rollback and adds two narrowly scoped Quick Stack features. Direct observation of an owned, opened or explicitly targeted vanilla chest records versioned local per-player/per-world item-to-chest hints; a zero-stock hint may be used only when no currently matching chest accepts any units, and only after the exact loaded chest is freshly rediscovered and validated. Quick Stack now highlights only the eligible quantity that it attempted but left behind, follows the exact surviving item object through ordinary sorting, clears on that item's first hover or inventory close, and is replaced by the next accepted Quick Stack. The visible BepInEx setting is **Quick Stack keybind** and migrates custom values from the legacy setting name. No player item custom data, per-frame scans, display caches, inventory-change requirement refreshes, or removed 1.1.7 performance machinery were added. This candidate makes no performance-fix claim and still requires live comparison; 1.1.5 remains production.
...
Current 1.1.9 verification covers 155 pure-domain tests, 283 independent compatibility/API/IL checks, and 77 static/repository safety checks (515 total). It extends the 1.1.8 regression suite with pure-domain coverage for strict remembered-destination serialization, malformed data, replacement, zero-stock retention, retention/cap pruning, exact per-stack fallback routing, current-route precedence, partial-route behavior, inert stale/ineligible hints, failed-deposit eligibility, and partial failed quantities. Compatibility and repository checks cover the pointer-driven acknowledgment hook, player-grid identity, exact source-item and ZDO identity/revision checks before every move, post-mutation revision advancement, lifecycle cleanup, and the continued absence of removed display-performance machinery. Reproducible package hashes are recorded at release-gate time. Joe live-tested and confirmed the 1.1.5 crafting debit fix; the 1.1.8 rollback and all 1.1.9 additions remain live-unverified. Broader multiplayer behavior, dedicated-server behavior, workbench extensions, manual-access preemption, and wider topology/load-unload edge cases remain independently unverified.
