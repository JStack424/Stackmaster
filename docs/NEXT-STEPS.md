...
The **1.1.4 Harmony-target fix** carries forward the released 1.1.0 gameplay and configuration unchanged and supersedes 1.1.3. Runtime identity is diagnostic only: exact Valheim labels, hashes, MVIDs, and component versions do not decide compatibility. A single 32-operation manifest now declares every target and patch entrypoint consumed by both compatibility validation and installation, including the cleanup-safety subset used during hot unload. Version 1.1.4 corrects `InventoryGui.SetupRequirement` as the one static Harmony target and validates each exact declaring type, overload, static/instance shape, return type, and patch signature before the first patch.
...
Current 1.1.4 verification covers 133 pure-domain tests, 258 independent compatibility/API/IL checks, and 69 static/repository safety checks. In addition to the 1.0.0 policy matrix, the suite covers the separate configured-modifier left/right inventory routes, no-dialog left-click protection, full target clearing on unlock, right-click add/edit/cancel semantics, non-stackable no-change behavior, ordinary-click passthrough, expedition modifier-only interception, carried-material exclusion, repeated full kits, largest-stock-first deterministic withdrawal, split sources, atomic shortages, shared-slot capacity simulation, partial-stack filling, carry-weight boundaries, the exact active build-menu hook, compatibility-gated runtime APIs, post-ownership replanning, reservation/revalidation, rollback, and immediate ownership cleanup. Joe's in-game validation of the 1.1.0 gameplay, including the protected-item controls and expedition-kit action, passed. The unchanged-gameplay 1.1.4 Harmony-target correction still requires its startup, compatibility-gate, and patch-installation smoke test. Broader multiplayer behavior, dedicated-server behavior, workbench extensions, manual-access preemption, and wider topology/load-unload edge cases remain independently unverified.
...
- Stackmaster 1.1.4 is the corrected Harmony-target build and supersedes 1.1.3. Its exact committed source is pushed to the existing GitHub `main`; no tag, GitHub Release, or Thunderstore upload is part of this work.
...
- Automated verification passes: zero compiler warnings/errors, 133/133 pure-domain tests, 258/258 compatibility/API/IL checks, and 69/69 static/repository safety checks.
- Joe's in-game validation of the 1.1.0 gameplay passed. The unchanged-gameplay 1.1.4 Harmony-target fix, dedicated servers, workbench extensions, manual-access preemption, and broader multiplayer/topology/load-unload behavior remain independently unverified.

**1.1.4 live-test checklist:**

0. Start Valheim with 1.1.4 and confirm the compatibility gate and Harmony installation both pass. If either fails, capture the complete BepInEx log before continuing.
...
**Next handoff:** Install and live-test `JStack424-Stackmaster-1.1.4-test.zip` with the checklist above. Confirm startup, chest-backed crafting and cleanup, disconnect/rejoin rearming, expedition-kit withdrawals, Alt+Left protection toggling, Alt+Right target editing without ever clearing a lock, the open-chest shortcut, free-fly blocking, and ward protection. Do not tag, create a GitHub Release, or upload this build to Thunderstore as part of this compatibility-only fix.
