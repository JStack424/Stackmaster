...
The **1.1.3 compatibility-gate fix** carries forward the released 1.1.0 gameplay and configuration unchanged and supersedes 1.1.2. Runtime identity is diagnostic only: exact Valheim labels, hashes, MVIDs, and component versions do not decide compatibility. The mod validates its complete required member contract and every exact Harmony target/entrypoint before the first patch, then disables and unpatches fully on any failure. Version 1.1.3 corrects four contracts that mistakenly requested instance methods even though Stackmaster calls—and the pinned client assemblies expose—static methods.
...
- Stackmaster 1.1.3 is the corrected compatibility-gate build and supersedes 1.1.2. Its source is intended for the existing GitHub main branch; no tag, GitHub Release, or Thunderstore upload is part of this work.
...
- Automated verification passes: zero compiler warnings/errors, 133/133 pure-domain tests, 98/98 compatibility/API/IL checks, and 66/66 static/repository safety checks.
- Joe's in-game validation of the 0.4.2 release candidate passed. The unchanged-gameplay 1.1.3 compatibility fix, dedicated servers, workbench extensions, manual-access preemption, and broader multiplayer/topology/load-unload behavior remain independently unverified.

**1.1.3 live-test checklist:**

0. Start Valheim with 1.1.3 and confirm Stackmaster no longer disables itself with false `GetSessionID`, `InFreeFly`, `ResetButtonStatus`, or `CheckAccess` missing-contract errors. If any compatibility error remains, capture the complete BepInEx log before continuing.
...
**Next handoff:** Install and live-test `JStack424-Stackmaster-1.1.3-test.zip` with the checklist above. Do not tag, create a GitHub Release, or upload this build to Thunderstore as part of this compatibility-only fix.
