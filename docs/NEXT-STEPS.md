...
The **1.1.0 local test candidate** adds an atomic expedition-kit action to the current `BuildUi.OnSelectPiece(Piece)` path. Holding the configured storage-action modifier (Left Alt by default) while clicking a build piece withdraws one complete recipe from eligible storage into the player inventory and suppresses vanilla piece selection/menu close for that click. Ordinary clicks remain vanilla. Every modified click requests another complete kit without credit for carried materials. Each ingredient is drawn from the chest with the largest total stock first, then smaller sources with deterministic ties. The action moves nothing unless storage has the complete kit, the exact resulting stacks fit, the complete added weight stays within carry capacity, all required chests can be acquired through the existing owner-authorized handshake, and fresh post-ownership planning, reservations, identities, access, use state, revisions, stacks, and quantities all match. Expedition reservations and proven acquired ownership are released immediately and do not create or renew the 30-second building lease. No Jötunn, custom RPC, server data, or networking was added. This candidate is not published and awaits live testing.
...
Current 1.1.0 candidate verification passes with zero compiler warnings/errors, 111/111 pure-domain tests, and 62/62 static/repository safety checks. In addition to the 1.0.0 policy matrix, the suite covers modifier-only interception, carried-material exclusion, repeated full kits, largest-stock-first deterministic withdrawal, split sources, atomic shortages, shared-slot capacity simulation, partial-stack filling, carry-weight boundaries, the exact active build-menu hook, compatibility-gated runtime APIs, post-ownership replanning, reservation/revalidation, rollback, and immediate ownership cleanup. Joe's in-game validation of the earlier 0.4.2 release candidate passed; live validation of the 1.1.0 expedition-kit action is pending. Broader multiplayer behavior, dedicated-server behavior, workbench extensions, manual-access preemption, and wider topology/load-unload edge cases remain independently unverified.
...
- Automated verification passes: zero compiler warnings/errors, 111/111 pure-domain tests, and 62/62 static/repository safety checks.
- Joe's in-game validation of the 0.4.2 release candidate passed. The 1.1.0 expedition-kit action, dedicated servers, workbench extensions, manual-access preemption, and broader multiplayer/topology/load-unload behavior remain independently unverified.

**1.1.0 expedition-kit live-test checklist:**

1. Open the current build menu, hold Left Alt, and click a build piece. Confirm one complete recipe moves from eligible storage into the player inventory and the build menu stays open without changing the selected piece.
2. Click the same piece again while still holding Left Alt. Confirm another complete kit is added even though one kit is already carried.
3. Release Left Alt and click normally. Confirm vanilla piece selection, sound, and menu close are unchanged.
4. Put one ingredient across several eligible chests with different totals. Confirm the largest-stock chest is drained first, then smaller sources; repeat equal-stock ties and confirm the same deterministic order.
5. Test a recipe split across multiple chests, including a remote-owned chest, and confirm the full kit arrives after ownership preparation with no retry click.
6. Make one ingredient short, one required chest busy, one chest inaccessible, the inventory one slot too small, and carry capacity one unit too low. Confirm every case moves nothing and changes no unrelated item or chest.
7. Test a partial compatible player stack plus a required new stack, and a recipe whose ingredients compete for the same last empty slot. Confirm successful packing in the first case and atomic cancellation in the second.
8. Change a required chest's contents or ownership during acquisition. Confirm the stale action cancels with no partial movement.
9. Confirm expedition success and every failure release acquired ownership immediately, while an unrelated pre-existing 30-second chest-backed building lease remains intact.
10. Disable/unload Stackmaster during a pending remote acquisition and confirm no items move afterward and no ownership is stranded. Repeat as a client-only mod on a vanilla host/server when practical.
