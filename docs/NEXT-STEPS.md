...
- v0.1 clean-profile solo smoke gate passed. Test Build 2 launched and passed Joe's initial nearby-chest quantity check; Test Builds 3–4 introduced aggregate build/craft HUD support, with the current static crafting-row hook corrected in Test Build 8.
- Local 0.2.0 Test Build 8 is the current unpublished candidate. Its crafting requirement postfix now matches the current game’s static rendering method and resolves the active GUI explicitly, so crafting rows receive the same aggregate text/color behavior as building rows. It reads ownership-independent serialized chest snapshots for display/planning, hydrates detached item metadata so valid stock is countable, rejects any chest with an incomplete detached item, retains the null live-inventory and fail-open UI protections, and requests only the minimum remotely owned chests required by an exact player-first action plan.
...
- Automated verification for Test Build 8 passes: 55/55 pure-domain tests and 45/45 static/repository checks, with zero compiler warnings/errors.
...
**Required 0.2.0 Test Build 8 live checks:**
...
**Next handoff:** Install the local 0.2.0 Test Build 8 ZIP and first confirm workbench, forge, cauldron, and upgrade requirement rows show `required / total available`, aggregate-satisfied rows do not flash red, true shortages still flash red, chest opening remains normal, and nearby contents remain counted; then begin checks 1–5 using two players so ownership can be observed directly. Do not change the live Thunderstore package or GitHub release until Joe explicitly approves a later publication step.
