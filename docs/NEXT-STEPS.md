...
The **1.1.2 compatibility-gate fix** carries forward the released 1.1.0 gameplay and configuration unchanged. Runtime identity is diagnostic only: exact Valheim labels, hashes, MVIDs, and component versions no longer decide compatibility. The mod now validates its complete required member contract and every exact Harmony target/entrypoint before the first patch, then disables and unpatches fully on any failure.
...
- Stackmaster 1.1.2 is the compatibility-gate fix. Its source is intended for the existing GitHub main branch; no tag, GitHub Release, or Thunderstore upload is part of this work.
...
- Joe's in-game validation of the 0.4.2 release candidate passed. The unchanged-gameplay 1.1.2 compatibility fix, dedicated servers, workbench extensions, manual-access preemption, and broader multiplayer/topology/load-unload behavior remain independently unverified.

**1.1.2 live-test checklist:**
...
**Next handoff:** Install and live-test `JStack424-Stackmaster-1.1.2-test.zip` with the checklist above. Do not tag, create a GitHub Release, or upload this build to Thunderstore as part of this compatibility-only fix.
