...
The **replacement 1.1.8 local test candidate** selectively rolls requirement-display performance behavior back to the straightforward 1.1.6 baseline after Joe's live test showed that the 1.1.7/earlier-1.1.8 cache experiment made hammer-related hitching worse and added lag when opening the inventory or moving items between slots. The bounded display epochs, cross-caller and long-lived chest reuse, indexed totals, player-inventory fingerprints/dirty events, and movement/topology reuse machinery are gone. The independent 1.1.6 outside-workbench player-only crafting bypass (with Valheim deciding complete player cost), reader-first docs, Quick Grab Materials terminology, keyboard/mouse build-menu hint, and no-op auto-sort notification guard remain. This candidate makes no performance-fix claim and still requires live comparison; 1.1.5 remains production.
...
Current 1.1.8 automated verification covers 144 pure-domain tests, 275 independent compatibility/API/IL checks, and 73 static/repository safety checks. In addition to the prior policy matrix, the suite now proves that the failed display-epoch/index/fingerprint machinery is absent, the 1.1.6 same-frame raw-accounting path is present, Valheim retains the player-only complete-cost verdict, no-op auto-sort avoids a synthetic inventory-change notification, and the keyboard-only Quick Grab Materials hint lifecycle remains covered. Joe live-tested and confirmed the 1.1.5 crafting debit fix; the replacement 1.1.8 selective rollback and retained hint remain live-unverified. Broader multiplayer behavior, dedicated-server behavior, workbench extensions, manual-access preemption, and wider topology/load-unload edge cases remain independently unverified.
