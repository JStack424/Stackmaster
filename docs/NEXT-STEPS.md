...
The **1.1.0 local test candidate** adds an atomic expedition-kit action to the current `BuildUi.OnSelectPiece(Piece)` path and replaces the old all-in-one protected-item modifier-click prompt. In the player inventory, the configured modifier now has distinct mouse-button paths: modifier + left-click instantly protects an unprotected item without a target, or fully unprotects any protected item and clears its target; modifier + right-click opens target entry for stackable items, with confirmation adding/editing the target and cancellation preserving the exact prior state. Non-stackable modifier-right-clicks are consumed without changing protection, and unmodified clicks remain vanilla. In the build menu, holding that same configured modifier (Left Alt by default) while clicking a build piece withdraws one complete recipe from eligible storage into the player inventory and suppresses vanilla piece selection/menu close for that click. Every modified build-piece click requests another complete kit without credit for carried materials. Each ingredient is drawn from the chest with the largest total stock first, then smaller sources with deterministic ties. The action moves nothing unless storage has the complete kit, the exact resulting stacks fit, the complete added weight stays within carry capacity, all required chests can be acquired through the existing owner-authorized handshake, and fresh post-ownership planning, reservations, identities, access, use state, revisions, stacks, and quantities all match. Expedition reservations and proven acquired ownership are released immediately and do not create or renew the 30-second building lease. No Jötunn, custom RPC, server data, or networking was added. This candidate is not published and awaits live testing.
...
Current 1.1.0 candidate verification passes with zero compiler warnings/errors, 120/120 pure-domain tests, and 62/62 static/repository safety checks. In addition to the 1.0.0 policy matrix, the suite covers the separate configured-modifier left/right inventory routes, no-dialog left-click protection, full target clearing on unlock, right-click add/edit/cancel semantics, non-stackable no-change behavior, ordinary-click passthrough, expedition modifier-only interception, carried-material exclusion, repeated full kits, largest-stock-first deterministic withdrawal, split sources, atomic shortages, shared-slot capacity simulation, partial-stack filling, carry-weight boundaries, the exact active build-menu hook, compatibility-gated runtime APIs, post-ownership replanning, reservation/revalidation, rollback, and immediate ownership cleanup. Joe's in-game validation of the earlier 0.4.2 release candidate passed; live validation of the 1.1.0 protected-item controls and expedition-kit action is pending. Broader multiplayer behavior, dedicated-server behavior, workbench extensions, manual-access preemption, and wider topology/load-unload edge cases remain independently unverified.
...
- Stackmaster 0.4.0 remains the current Thunderstore release until Joe uploads an approved replacement.
- Stackmaster 1.1.0 is a local-only test candidate. It has not been pushed, tagged, released on GitHub, or uploaded to Thunderstore.
...
- Automated verification passes: zero compiler warnings/errors, 120/120 pure-domain tests, and 62/62 static/repository safety checks.
- Joe's in-game validation of the 0.4.2 release candidate passed. The 1.1.0 protected-item control redesign, expedition-kit action, dedicated servers, workbench extensions, manual-access preemption, and broader multiplayer/topology/load-unload behavior remain independently unverified.

**1.1.0 live-test checklist:**
...
**Next handoff:** Install and live-test `JStack424-Stackmaster-1.1.0-test.zip` with the checklist above. Do not push, tag, create a GitHub Release, or upload this candidate to Thunderstore unless Joe explicitly approves it after testing.
