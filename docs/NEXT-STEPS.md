...
Current 1.1.5 verification covers 137 pure-domain tests, 262 independent compatibility/API/IL checks, and 70 static/repository safety checks. In addition to the prior policy matrix, the suite now covers exact mixed player/chest debit, selected live-chest identity, one-shot charge protection, incomplete-debit rejection, exact cancellation/failure rollback, and the pinned `DoCrafting` output-before-charge call shape for both the direct string-removal and `Player.ConsumeResources` paths. Existing coverage for protection, expedition kits, storage scope, ownership, reservation, cleanup, and reconnect behavior also remains green. Joe's in-game validation of the 1.0.0-era gameplay passed, but the 1.1.5 crafting fix itself still requires live startup, chest-only and mixed crafting, multicraft, upgrade, cancellation, disconnect/rejoin, and ownership-cleanup validation. Broader multiplayer behavior, dedicated-server behavior, workbench extensions, manual-access preemption, and wider topology/load-unload edge cases remain independently unverified.
...
- Stackmaster 1.1.5 is the local test-only chest-backed crafting transaction fix and supersedes the unapproved 1.1.4 candidate. Its source must remain local until Joe's live approval; do not push GitHub, tag, create a GitHub Release, build a production ZIP, or upload Thunderstore.
...
- Joe's in-game validation of the 1.1.0 gameplay passed. The 1.1.5 live-inventory crafting fix, dedicated servers, workbench extensions, manual-access preemption, and broader multiplayer/topology/load-unload behavior remain independently unverified.
...
7. Click the same piece again while still holding Left Alt. Confirm another complete kit is added even though one kit is already carried.
...
10. Test a recipe split across multiple chests, including a remote-owned chest, and confirm the full kit arrives after ownership preparation with no retry click.
...
14. Confirm expedition success and every failure release acquired ownership immediately, while an unrelated pre-existing 30-second chest-backed building lease remains intact.
...
**Next handoff:** Install and live-test `JStack424-Stackmaster-1.1.5-test.zip` with the checklist above. Lead with chest-only and mixed player/chest crafting, exact quantities, no double charge, multicraft, upgrades, cancellation, disconnect/rejoin, and ownership cleanup; then spot-check the carried features. On any failure, capture the full Stackmaster/BepInEx log and exact before/after inventories. Do not push, tag, create a GitHub Release or production ZIP, or upload this test candidate to Thunderstore before Joe approves it.
