...
The **1.2.0 production release** preserves the completed selective requirement-display rollback and adds two narrowly scoped Quick Stack features. Direct observation of an owned, opened or explicitly targeted vanilla chest records versioned local per-player/per-world item-to-chest hints; a zero-stock hint may be used only when no currently matching chest accepts any units, and only after the exact loaded chest is freshly rediscovered and validated. Quick Stack now highlights only the eligible quantity that it attempted but left behind, follows the exact surviving item object through ordinary sorting, clears on that item's first hover or inventory close, and is replaced by the next accepted Quick Stack. Its deposited, replenished, and left-behind totals are shown as an unclipped multiline bullet list, including zeros. The visible BepInEx setting is **Quick Stack keybind** and migrates custom values from the legacy setting name. No player item custom data, per-frame scans, display caches, inventory-change requirement refreshes, or removed performance-experiment machinery were added. Joe live-tested and approved the Quick Stack behavior before productionization; the release deliberately makes no performance-improvement claim.

The **1.1.5 crafting transaction fix** supersedes the unapproved 1.1.4 candidate. Version 1.1.4 planned exact player/chest costs correctly, but its prepared crafting path captured detached read-only ZDO inventory snapshots again after ownership acquisition and removed chest-backed costs from those throwaway copies. Vanilla could then create the item while Stackmaster suppressed vanilla's duplicate charge. Version 1.1.5 recaptures selected containers through each live owned `Container.GetInventory()`, requires the live plan to match the original exact plan, reserves and revalidates that identity, and journals every observed mutation. An incomplete debit cannot commit and is rolled back. The pinned release gate now also proves `InventoryGui.DoCrafting(Player)` still calls the exact string-based `Inventory.RemoveItem` path and `Player.ConsumeResources` after its output-add call shape. Joe live-tested the corrected chest-backed debit and approved this exact gameplay candidate for production.

The **1.0.0 production release** promotes the live-tested 0.4.2 candidate without gameplay or configuration changes. It carries forward the complete 0.4.1 behavior and splits storage-aware requirements into three default-on settings: **Allow crafting from storage**, **Allow building from storage**, and **Show storage amounts in craft and build menus**. The display setting independently controls exact `required / total available` text in crafting, upgrade, and building rows. Affordability and red/white flashing use player-held stock when the relevant action permission is off, even if the visible aggregate total is sufficient; when that action permission is on they use combined player-plus-eligible-storage stock, even if aggregate text is hidden. Turning both display and the relevant permission off leaves that requirement UI untouched. The existing connected-workbench mesh / fallback scope and all consumption paths remain unchanged. The release also retains 0.4.1's 30-second successful-build ownership lease, manual-access preemption, adaptive long requirement-count fit, protected-item exit cleanup, and orphan pruning.

The **0.4.0 previous production release** replaced four separate notions of “nearby” with one shared storage-scope resolver. Inside a canonical vanilla workbench build zone, it seeds from every zone containing the local player, traverses every transitively overlapping loaded workbench zone, and includes loaded supported vanilla containers only inside that exact connected union. It uses each station's current `GetStationBuildRange()` result and Valheim's horizontal, strict-boundary semantics rather than a hardcoded radius. Outside every valid mesh, deposit, replenishment, building, and crafting all use the existing configurable player-centered fallback radius (20 metres by default). Explicit-target-first and nearest routing, player-first exact minimum-chest resource planning, and loaded-only discovery remain unchanged. The per-chest `Auto-sort chest` checkbox affects sorting only and never removes a chest from this shared scope.

The mesh resolver uses the canonical `piece_workbench` prefab resolved through `ZNetScene`, verifies each loaded station's ZDO prefab identity, and is covered by the exact runtime compatibility gate. Mesh discovery is complete rather than subject to the ordinary nearby-action responsiveness cutoff. Read-only resource display still claims nothing. After any asynchronous ownership yield and again before mutation, actions recompute a single immutable scope snapshot and validate selected chest membership together with identity, access, use state, revisions, quantities, and ownership. Fresh action captures resolve current location and connected-workbench topology before any mutation. Requirement displays again follow the 1.1.6 same-frame capture behavior; no long-lived display topology or chest snapshot is retained across frames.

Current 1.2.0 verification covers 158 pure-domain tests, 283 independent compatibility/API/IL checks, and 78 static/repository safety checks (519 total). It covers strict remembered-destination serialization, malformed data, replacement, zero-stock retention, retention/cap pruning, exact per-stack fallback routing, current-route precedence, partial-route behavior, inert stale/ineligible hints, failed-deposit eligibility, partial failed quantities, and the exact multiline result format with zero values and stable wording. Compatibility and repository checks cover the pointer-driven acknowledgment hook, player-grid identity, exact source-item and ZDO identity/revision checks before every move, post-mutation revision advancement, lifecycle cleanup, direct top-left delivery of the multiline message without truncation, and the continued absence of removed display-performance machinery. Reproducible package hashes are recorded at release-gate time. Joe live-tested and approved the 1.2.0 Quick Stack behavior; broader multiplayer behavior, dedicated-server behavior, workbench extensions, manual-access preemption, and wider topology/load-unload edge cases remain independently unverified.
...
Valheim's owner-authorized response is asynchronous, so it cannot safely finish inside a synchronous Harmony build/craft prefix. Blind `ClaimOwnership` is not used. A remote-owned action cancels its first attempt without mutation while ownership is prepared, then asks the player to retry through the normal vanilla action path. Stackmaster keeps only ownership it demonstrably acquired available for a bounded 10-second retry window. A fully confirmed successful building transaction promotes only its exact used acquired chests to the 30-second sliding build lease; crafting never does. Reservations and any rollback finish before ownership is retained or released. Every non-success cleanup path returns the exact ZDO to Valheim's native unowned state immediately when the existing guards still prove that safe, and otherwise retries later rather than touching uncertain ownership. A required chest rejected as busy or unavailable shows exactly `The required materials are currently in use`; denial, stale state, a changed plan, or any failed validation cancels without partial consumption.

Joe live-approved the corrected gameplay candidate on September 12, 2026 and confirmed it works solo and as a client-only mod connected to a vanilla server, with no Stackmaster installation required for the host, server, or other players. That release candidate's automated verification passed with zero compiler warnings/errors, 61/61 pure-domain tests, and 48/48 static/repository checks. These results and completed live checks do **not** establish universal multiplayer or network safety: broader co-op host, co-op guest, and unmodded dedicated-server coverage remains incomplete, and the public README retains a concise early-testing note.
...
- The configured storage-action modifier manages one-record/one-matching-stack protection through separate mouse paths. Modifier + left-click is an immediate protection-only toggle with no dialog; unlocking removes any target. Modifier + right-click opens target quantity entry only for stackable items, prefilling an existing target when present or the legal full-stack target otherwise. Confirmation adds or edits the target; cancel and invalid input preserve prior state. Preferred-slot matches win; moved stacks reattach deterministically; unrelated replacements never inherit.
...
- [x] Exact installed Valheim Steam build and executable version: build `25364309`, Unity `6000.0.75f1`.
...
- [x] Add configured-modifier + left-click instant protect/unprotect and modifier + right-click stackable target-entry behavior, with exact cancel preservation and vanilla ordinary clicks.
...
- [x] The exact ZIP contains valid required Thunderstore files and no game assemblies, local paths, credentials, tests, source, PDBs, or development debris.
...
- Stackmaster 1.1.0 remains the current Thunderstore release until Joe uploads an approved replacement.
- Stackmaster 1.1.5 is the live-approved chest-backed crafting transaction fix and supersedes the unapproved 1.1.4 candidate. Its exact tested gameplay source is productionized only through release packaging and documentation changes; push the matching committed source to GitHub `main`, but do not create a tag, GitHub Release, or Thunderstore upload.
- Stackmaster 1.0.0 remains the approved production build promoted from the live-tested 0.4.2 candidate without gameplay or configuration changes. Its exact committed source is published to GitHub `main`; no Git tag or GitHub Release is part of this workflow.
- The release retains only exact Valheim ownership demonstrably acquired and actually used by a fully confirmed successful chest-backed building placement. The lease lasts 30 seconds from the latest successful use of that chest.
- Logical `in-use` reservations end immediately. Crafting, failure, cancellation, rollback, partial or unused acquisition, disable, disconnect, logout, scene unload, shutdown, hot unload, compatibility disable, and exceptions retain no build lease.
- Accepted remote manual chest access invalidates an idle build lease as vanilla transfers ownership; local manual access remains available without preemption.
- Exact identity, acquiring-session, current-local-owner, and successor owner-revision guards remain mandatory. Expiry returns the proven acquisition to Valheim owner `0`; unrelated, merely inspected, already-local, and non-acquired chests are never released.
- Connected-workbench mesh / 20 m fallback scope, player-first exact accounting, minimum distinct-chest planning, auto-sort opt-outs, rollback, and the exact message `The required materials are currently in use` are unchanged.
- Crafting and upgrade totals preserve all digits with normal-size text when it fits, bounded auto-sizing for longer values, and full vanilla typography restoration across pooled-row reuse, toggle-off, compatibility-disable, and unload paths.
- Configured-modifier + left-click now protects an unprotected player-inventory item immediately without a target or dialog, while the same click on any protected item removes both protection and target. Configured-modifier + right-click is the only target editor, preserves prior state on cancel, and leaves non-stackable protection unchanged.
- Manually transferring a whole protected/targeted stack into a chest or dropping it into the world clears that stack's record only after the item has actually left the player inventory. Partial external moves retain the protected remainder, while internal moves, merges, splits, and transient drags keep item-following intact.
- Storage-hotkey planning prunes older orphaned target records that cannot resolve any compatible player-inventory item, suppressing stale `target item missing` notices without disturbing valid moved targets.
- Automated verification passes: zero compiler warnings/errors, 137/137 pure-domain tests, 262/262 compatibility/API/IL checks, and 70/70 static/repository safety checks.
- Joe's in-game validation of the corrected 1.1.5 chest-backed crafting debit passed. Dedicated servers, workbench extensions, manual-access preemption, and broader multiplayer/topology/load-unload behavior remain independently unverified.

**1.1.5 crafting-fix live-test checklist:**

0. Start Valheim with 1.1.5 and confirm the compatibility gate and Harmony installation both pass. If either fails, capture the complete BepInEx log before continuing.
1. Craft with every required unit in eligible chests and none in the player inventory. Verify the crafted item appears and the exact chest quantities disappear.
2. Craft with one recipe cost split between the player inventory and one or more eligible chests. Verify each source loses exactly its planned share and no source is charged twice.
3. Repeat with multi-craft and an upgrade whose quality-level cost differs from the base recipe. Verify exact multiplied/quality-adjusted totals.
4. Start and cancel a craft before completion. Verify player and chest inventories remain unchanged and acquired chest ownership is released.
5. Force an unavailable, busy, changed, or disconnected required chest during preparation/completion. Verify no free output, no partial retained charge, and no stranded ownership.
6. Disconnect/rejoin and repeat one chest-only craft, then verify ordinary chest access and ownership cleanup still work.
7. Repeat with **Allow crafting from storage** disabled and with vanilla NoCost enabled. Verify both remain vanilla and Stackmaster neither debits nor suppresses unexpectedly.

**Carried-forward feature regression checklist:**

1. In the player inventory, hold Left Alt and left-click an unprotected stackable item. Confirm it locks immediately with no dialog and no target quantity.
2. Repeat modifier-left-click on both a protection-only item and a targeted item. Confirm each fully unlocks in one click and the targeted item loses its target.
3. Hold Left Alt and right-click an unprotected stackable item. Confirm the quantity dialog opens; enter a legal value and confirm the item becomes protected with that target.
4. Modifier-right-click that targeted item again. Confirm its current target is prefilled, edit it, and verify the target changes without removing protection. Reopen and cancel; confirm the exact existing protection and target remain.
5. Modifier-right-click an unprotected and a protected non-stackable item. Confirm neither receives a target or changes protection state. Exercise ordinary left/right clicks and chest-grid clicks without Left Alt and confirm all remain vanilla.
6. Open the current build menu, hold Left Alt, and click a build piece. Confirm one complete recipe moves from eligible storage into the player inventory and the build menu stays open without changing the selected piece.
7. Click the same piece again while still holding Left Alt. Confirm another complete material set is added even though one set is already carried.
8. Release Left Alt and click normally. Confirm vanilla piece selection, sound, and menu close are unchanged.
9. Put one ingredient across several eligible chests with different totals. Confirm the largest-stock chest is drained first, then smaller sources; repeat equal-stock ties and confirm the same deterministic order.
10. Test a recipe split across multiple chests, including a remote-owned chest, and confirm all required materials arrive after ownership preparation with no retry click.
11. Make one ingredient short, one required chest busy, one chest inaccessible, the inventory one slot too small, and carry capacity one unit too low. Confirm every case moves nothing and changes no unrelated item or chest.
12. Test a partial compatible player stack plus a required new stack, and a recipe whose ingredients compete for the same last empty slot. Confirm successful packing in the first case and atomic cancellation in the second.
13. Change a required chest's contents or ownership during acquisition. Confirm the stale action cancels with no partial movement.
14. Confirm quick-grab success and every failure release acquired ownership immediately, while an unrelated pre-existing 30-second chest-backed building lease remains intact.
15. Disable/unload Stackmaster during a pending remote acquisition and confirm no items move afterward and no ownership is stranded. Repeat as a client-only mod on a vanilla host/server when practical.

**0.4.2 release-candidate validation checklist:**

1. Exercise all eight combinations of **Allow crafting from storage**, **Allow building from storage**, and **Show storage amounts in craft and build menus**.
2. In crafting and upgrade rows, confirm visible totals always show player plus eligible storage when enabled, but red/white flashing follows player-only stock when crafting storage use is off and aggregate stock when it is on.
3. Repeat the same affordability checks in the separate build-menu requirement path using the building permission.
4. With totals off, confirm vanilla count text and typography remain untouched while enabled storage-backed actions still work and show affordability correctly. With both totals and the relevant permission off, confirm the full row remains vanilla.
5. Check short and long values including `2 / 17`, `45 / 172`, and `999 / 999`, then switch back to a short value and confirm normal typography returns.
6. Confirm crafting and building consumption remain independently controlled, exact, player-first, and all-or-nothing.

**Carried-forward 0.4.1 validation checklist:**

1. Acquire a remote chest through the first attempt, retry one chest-backed build successfully, and confirm rapid subsequent placements use the retained ownership without another preparation click.
2. Continue building from the same chest and verify each successful placement slides the release deadline to 30 seconds after its latest use.
3. Stop using the chest and verify it returns to Valheim owner `0` after lease expiry, with vanilla peers able to open it.
4. Have another remote player manually open a leased, idle chest before expiry; confirm Stackmaster does not block the request and the lease does not later overwrite vanilla's transfer.
5. Confirm the local owner can manually open and close a leased chest normally.
6. Exercise cancellation, failed placement, rollback, partial/unused acquisition, settings disable, disconnect, logout, scene unload, shutdown, hot unload, compatibility disable, and exception cleanup; no ownership may remain stranded.
7. Confirm crafting never receives the 30-second build lease, and unrelated, inspected-only, already-local, or non-acquired chests are never retained or released.
8. Open crafting and upgrade rows showing `45 / 172` and `999 / 999`; verify every digit stays visible, then switch to a short value and verify its normal font size returns. Disable nearby crafting and confirm vanilla typography is fully restored.
9. Manually move a whole protected/targeted stack into a chest, then repeat by dropping one into the world; confirm each removed stack's protection clears without affecting other protected items.
10. Partially transfer and partially drop protected stacks; confirm the compatible remainder stays protected and targeted. Also confirm ordinary player-inventory moves, merges, splits, slot changes, transient drags, and failed transfers preserve protection.
11. Remove a previously targeted item, trigger the storage hotkey, and confirm the stale record is silently pruned with no `target item missing` shortage while valid moved targets still replenish normally.
12. Repeat exact accounting, minimum-chest selection, connected-workbench and 20 m fallback scope, auto-sort opt-out, multi-craft/quality, deposit, replenishment, protection, and busy-message regressions.

**Next handoff:** Deliver the verified production `JStack424-Stackmaster-1.1.5.zip` to Joe for his Thunderstore upload after the exact committed source is pushed and independently confirmed on GitHub `main`. Do not create a Git tag or GitHub Release, and do not upload to Thunderstore on Joe's behalf.
