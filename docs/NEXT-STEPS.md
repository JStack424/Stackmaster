...
The **0.4.1 local r2modman test candidate** keeps Stackmaster's asynchronous first ownership-acquisition attempt, then retains only Valheim ownership demonstrably acquired and actually used by a successful chest-backed building placement on a 30-second sliding lease. Each later successful build use renews the exact chest's lease. The logical `in-use` reservation always ends immediately. A remote player's accepted manual open invalidates an idle build lease as vanilla transfers ownership; local manual access does not invalidate it. Crafting and every unsuccessful, cancelled, rolled-back, unused, disabled, disconnected, logged-out, unloaded, shutdown, hot-unloaded, compatibility-disabled, or exceptional path continue immediate guarded cleanup. Expiry returns the exact tracked ZDO to owner `0` only while identity, session, current-owner, and successor-revision guards still match. Crafting and upgrade requirement totals also preserve every digit: normal-size text is retained when it fits, while longer exact values use bounded TextMeshPro auto-sizing inside the existing row and restore vanilla typography before pooled-row reuse or when the feature disables. A whole protected stack manually transferred to a chest or dropped into the world clears its record only after the item has actually left the player inventory; partial external moves, failed transfers, transient drags, and moves or merges wholly inside the player inventory preserve protection. Storage-hotkey planning also prunes older target records that cannot resolve any compatible carried item, suppressing stale `target item missing` notices without affecting valid moved targets. This candidate is not published and does not claim multiplayer verification.
...
Current 0.4.1 automated verification passes with zero compiler warnings/errors, 95/95 pure-domain tests, and 55/55 static/repository safety checks. Core connected-workbench storage behavior from 0.4.0 has been live-tested in Valheim; the new 0.4.1 ownership lease, manual-access preemption, adaptive count fit, and protected-item exit lifecycle have not. Multiplayer behavior, dedicated-server behavior, workbench extensions, and broader topology/load-unload edge cases have not yet been independently verified, so this test candidate does not claim proven multiplayer safety.
...
- Stackmaster 0.4.1 is a local-only r2modman test candidate. Nothing in this workflow is tagged, pushed, released, or uploaded.
...
- Automated verification passes: zero compiler warnings/errors, 95/95 pure-domain tests, and 55/55 static/repository safety checks.
- The 0.4.1 lease behavior still needs live multiplayer validation. Dedicated servers, workbench extensions, manual-access preemption, and broader topology/load-unload behavior remain independently unverified.
...
**Next handoff:** Install `JStack424-Stackmaster-0.4.1-test.zip` locally through r2modman and run the validation above. Any GitHub push, tag/release, or Thunderstore publication remains a separate explicit action after live approval.
