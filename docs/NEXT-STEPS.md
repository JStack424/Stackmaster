...
Current 1.1.5 verification covers 137 pure-domain tests, 262 independent compatibility/API/IL checks, and 70 static/repository safety checks. In addition to the prior policy matrix, the suite now covers exact mixed player/chest debit, selected live-chest identity, one-shot charge protection, incomplete-debit rejection, exact cancellation/failure rollback, and the pinned `DoCrafting` output-before-charge call shape for both the direct string-removal and `Player.ConsumeResources` paths. Existing coverage for protection, Quick Grab Materials, storage scope, ownership, reservation, cleanup, and reconnect behavior also remains green. Joe live-tested the corrected chest-backed debit and confirmed it is fixed. Broader multiplayer behavior, dedicated-server behavior, workbench extensions, manual-access preemption, and wider topology/load-unload edge cases remain independently unverified.
