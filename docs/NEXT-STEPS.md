...
Current 1.1.9 verification covers 154 pure-domain tests, 283 independent compatibility/API/IL checks, and 77 static/repository safety checks (514 total). It extends the 1.1.8 regression suite with pure-domain coverage for strict remembered-destination serialization, malformed data, replacement, zero-stock retention, retention/cap pruning, exact per-stack fallback routing, current-route precedence, partial-route behavior, inert stale/ineligible hints, failed-deposit eligibility, and partial failed quantities. Compatibility and repository checks cover the pointer-driven acknowledgment hook, player-grid identity, exact ZDO identity/revision checks before every move, post-mutation revision advancement, lifecycle cleanup, and the continued absence of removed display-performance machinery. Reproducible package hashes are recorded at release-gate time. Joe live-tested and confirmed the 1.1.5 crafting debit fix; the 1.1.8 rollback and all 1.1.9 additions remain live-unverified. Broader multiplayer behavior, dedicated-server behavior, workbench extensions, manual-access preemption, and wider topology/load-unload edge cases remain independently unverified.
