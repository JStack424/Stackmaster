...
The current unpublished **0.2.0 Test Build 4** carries forward independently switchable building and crafting from eligible nearby chests; both default to enabled and share the existing configurable radius. Test Build 2 launched successfully and Joe confirmed an initial nearby-chest building check consumed the correct quantities. Test Build 3 added `required / total available` to the selected-piece HUD. Test Build 4 extends that same aggregate display and shortage coloring to every standard `InventoryGui.SetupRequirement` crafting/upgrade path, covering workbenches, forges, cauldrons, equivalent stations, selected upgrade quality, duplicate costs, one-ingredient quality constraints, and multi-craft totals. Its complete-action planner and withdrawal path are unchanged: they validate fresh chest/stack state, withdraw exact units before output/placement, suppress vanilla double charging, and roll back when vanilla does not complete the action.

Current local verification: zero compiler warnings/errors, 40/40 pure-domain tests, 40/40 static/repository checks, and only `Stackmaster.dll` plus `Stackmaster.pdb` in plugin output. The v0.1 solo smoke gate passed and v0.2 now has a successful startup plus initial nearby-build result, but the new crafting HUD and broader v0.2 paths still require the live-game checks below. These results do **not** establish multiplayer or network safety: co-op host, co-op guest, and unmodded dedicated-server validation remain mandatory before any proven multiplayer claim.
...
- Local 0.2.0 Test Build 4 is the current unpublished candidate. It carries forward the selected-piece HUD fix and extends `required / total available` plus aggregate shortage coloring to workbench, forge, cauldron, and equivalent crafting/upgrade menus without changing consumption logic.
- Automated verification for Test Build 4 passes: 40/40 pure-domain tests and 40/40 static/repository checks, with zero compiler warnings/errors; the crafting HUD behavior remains the immediate live check.
...
**Required 0.2.0 Test Build 4 live checks:**

1. At a workbench, forge, cauldron, and another available crafting station, select ordinary craft and upgrade recipes whose materials are split between inventory and eligible nearby chests; confirm each line reads `required / total available` (for example `2 / 15`) and stays white when the aggregate full requirement is satisfied. Exercise a multi-craft quantity and a one-ingredient recipe where available, then remove enough stock to confirm only the short material resumes the vanilla red blink.
2. Confirm the selected build-piece HUD still shows aggregate totals correctly. Turn nearby-chest crafting off and confirm all crafting/upgrade requirement rows and crafting behavior are strictly vanilla; independently turn nearby-chest building off and confirm the build HUD and behavior are strictly vanilla.
...
**Next handoff:** Install and run the live checklist with the local 0.2.0 Test Build 4 candidate, beginning with crafting/upgrade requirement totals and color behavior at each station type. Do not change the live Thunderstore or GitHub release until Joe explicitly approves a later publication step.
