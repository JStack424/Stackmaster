...
Before planning, the adapter reconciles each protection record to at most one compatible matching player stack, preferring its saved slot and otherwise choosing in deterministic row-major order. The sort planner then preserves the entire quick bar, equipped items, and currently resolved protected stacks; merges only compatible movable stacks; and orders resulting stacks by player-visible name. Storage planning keeps targeted-container and distance ordering explicit, fills compatible partial stacks before opening new destination stacks, reports shortages and leftovers, and conserves item-unit counts.
