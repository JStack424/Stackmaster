...
- In the player inventory, the configured storage-action modifier (Left Alt by default) has distinct mouse-button behavior: modifier + left-click instantly protects an unprotected item without a target, or fully unprotects any protected item and clears its target; modifier + right-click opens the restocking-target quantity dialog for stackable items. Confirming the right-click dialog protects the item and adds or edits its target, while cancel preserves the exact prior state. Modifier + right-click on a non-stackable item leaves its protection state unchanged. Ordinary clicks remain vanilla.
