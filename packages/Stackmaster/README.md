...
> **Compatibility status:** Stackmaster 1.1.5 is a test candidate fixing a serious 1.1.4 remote-crafting transaction bug: detached read-only chest snapshots could be debited instead of live inventories, granting chest-backed crafting resources for free. Prepared crafting now recaptures the exact plan from selected owned containers, verifies live inventory identity, journals every observed mutation, and fails closed unless the full exact debit is confirmed. The startup gate still validates required types, exact method overloads and parameters, fields, properties, and every Harmony target and patch entrypoint before patching; the release gate also verifies the pinned `DoCrafting` output/charge call shape used for duplicate-charge suppression. Runtime versions and assembly identity remain diagnostics only. This build still requires Joe's live startup, crafting, cancellation, cleanup, and multiplayer validation before release; back up valuable characters and worlds and report any item loss, duplication, free output, crash, blocked chest, or synchronization disagreement.
