...
> **Test candidate status:** Stackmaster 1.1.6 is a local multiplayer test candidate. Outside every workbench mesh, a craft fully funded by the player's live inventory now stays entirely on Valheim's vanilla validation and debit path, so unrelated nearby-player, storage, or workbench-topology changes cannot cancel a craft that never needed chest materials. Any chest-needed craft, every workbench-mesh craft, and every unresolved scope remains on Stackmaster's existing fail-closed transaction path. Stackmaster 1.1.5 remains the current production-ready build until this candidate passes live testing and is separately approved. The startup gate still validates required types, exact method overloads and parameters, fields, properties, and every Harmony target and patch entrypoint before patching; the release gate also verifies the pinned `DoCrafting` output/charge call shape used for duplicate-charge suppression. Runtime versions and assembly identity remain diagnostics only. Broader multiplayer, dedicated-server, workbench-extension, manual-access-preemption, and load/unload edge-case testing remains incomplete; back up valuable characters and worlds and report any item loss, duplication, free output, crash, blocked chest, or synchronization disagreement.
...
Version 1.1.6 supports keyboard and mouse. Controller-specific controls are not included yet.
