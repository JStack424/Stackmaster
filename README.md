...
> **Test candidate status:** Stackmaster 1.1.8 is a local performance and multiplayer test candidate. Requirement displays now pre-index material totals and reuse one complete read-only nearby-resource capture for a one-second display window instead of repeatedly running the exact withdrawal planner, resolving the same stable scope, discovering every container, and decoding detached inventories. If the player inventory changes, the next display query rebuilds only the player contribution after revalidating scope topology and membership; player-only refreshes cannot extend that chest data beyond a separate five-second hard bound. A topology change without a player-inventory event may remain visible only for the remainder of the one-second display window, while every action still resolves and validates fresh. Opening an inventory that is already sorted also avoids a no-op change notification and its unnecessary vanilla placement-ghost rebuild. Build, craft, upgrade, Quick Grab Materials, ownership, reservation, debit, and rollback paths always capture and validate fresh and cannot use the display index or cache. Pinned Valheim IL confirms that `Player.OnInventoryChanged` itself uses discovery, `IsKnown`, and `CanAlmostBuild` checks that Stackmaster skips; the Stackmaster-only load is the ordinary hammer HUD requirement loop surrounding Valheim's inventory-triggered recipe scan and placement-ghost recreation, not a direct chest scan inside `OnInventoryChanged`. This candidate targets that proven display overhead, but live frame-time improvement has not yet been measured and the vanilla ghost rebuild still occurs after real inventory changes. The 1.1.6 player-only fallback is retained and still defers its complete-cost verdict to Valheim itself, including ordinary quality-tier semantics. Stackmaster 1.1.5 remains the current production-ready build until this candidate passes live testing and is separately approved. Broader multiplayer, dedicated-server, workbench-extension, manual-access-preemption, and load/unload edge-case testing remains incomplete. Back up valuable characters and worlds and report any item loss, duplication, free output, crash, blocked chest, synchronization disagreement, or stale requirement total.
