...
> **Compatibility status:** Stackmaster 1.1.4 carries forward the live-tested 1.1.0 gameplay and configuration unchanged. Its startup gate now accepts differing client/server build labels, file hashes, and module IDs when the complete runtime API contract Stackmaster uses is intact. Before any patch is installed, it validates required types, exact method overloads and parameters, fields, properties, and every Harmony target and patch entrypoint; any missing, changed, or ambiguous contract disables Stackmaster, and any patch failure removes the whole patch set. Runtime versions and assembly identity are logged only as diagnostics. Automated coverage includes this fail-closed gate plus the existing gameplay, cleanup, and ownership safeguards. Broader multiplayer, dedicated-server, workbench-extension, and load/unload edge-case testing remains incomplete; back up valuable characters and worlds and report any item loss, duplication, crash, corrupt item data, blocked chest, or synchronization disagreement.
...
Version 1.1.4 supports keyboard and mouse. Controller-specific controls are not included yet.
